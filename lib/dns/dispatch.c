...
dispentry_destroy(dns_dispentry_t *resp) {
	...
	if (resp->handle != NULL) {
		dispentry_log(resp, LVL(90),
			      "detaching handle %p handle from %p",
...
udp_recv(isc_nmhandle_t *handle, isc_result_t eresult, isc_region_t *region,
	...
	dispentry_log(resp, LVL(90), "read callback:%s, requests %d", resp,
...
next:
	 ...
	 */
	response = NULL;
	udp_dispatch_getnext(resp, resp->timeout - dispentry_runtime(resp));
	...
	if (response != NULL) {
		dispentry_log(resp, LVL(90), "UDP read callback on %p: %s",
			      resp, isc_result_totext(eresult));
...
tcp_recv_success(dns_dispatch_t *disp, isc_region_t *region, dns_qid_t *qid,
	 ...
	 */
	if ((flags & DNS_MESSAGEFLAG_QR) == 0) {
		dispentry_log(resp, LVL(10), "got DNS query instead of answer");
		return (ISC_R_UNEXPECTED);
	}
...
tcp_recv(isc_nmhandle_t *handle, isc_result_t result, isc_region_t *region,
	...
	case ISC_R_TIMEDOUT:
		 ...
		 * active queue immediately, though, because the
		 * callback might decide to keep waiting and leave it
		 * active.)
		 */
...
dns_dispatch_gettcp(dns_dispatchmgr_t *mgr, const isc_sockaddr_t *destaddr,
	     ...
	     disp = ISC_LIST_NEXT(disp, link))
	{
		     ...
		     isc_sockaddr_eqaddr(localaddr, &sockname)))
		{
			...
			case DNS_DISPATCHSTATE_NONE:
			case DNS_DISPATCHSTATE_CONNECTING:
				/* We found "a" dispatch, store it for
				 * later */
			...
			case DNS_DISPATCHSTATE_CANCELED:
				/*
				 * We found a canceled dispatch, help
				 * its removal from the list, and skip
				 * it.
				 */
...
dispatch_createudp(dns_dispatchmgr_t *mgr, const isc_sockaddr_t *localaddr,
	...
	if (isc_log_wouldlog(dns_lctx, 90)) {
		...
		isc_sockaddr_format(localaddr, addrbuf,
				    ISC_SOCKADDR_FORMATSIZE);
		mgr_log(mgr, LVL(90),
			"dispatch_createudp: created UDP dispatch %p for "
			"%s",
			disp, addrbuf);
	}
	 ...
	 * Append it to the dispatcher list.
	 */
	/* FIXME: We don't care about UDP, only TCP should be searched */
	/* ISC_LIST_APPEND(mgr->list, disp, link); */
