...
static void
expire_name(dns_adbname_t *adbname, dns_adbstatus_t astat, isc_stdtime_t now);
static bool
...
shutdown_names(dns_adb_t *adb) {
	     ...
	     name = next)
	{
		 ...
		 * itself.
		 */
		expire_name(name, DNS_ADB_SHUTTINGDOWN, INT_MAX);
	}
...
maybe_expire_name(dns_adbname_t *adbname, isc_stdtime_t now) {
	...
	expire_name(adbname, DNS_ADB_EXPIRED, now);
...
purge_stale_names(dns_adb_t *adb, isc_stdtime_t now) {
	     ...
	     adbname = prev)
	{
		...
		if (overmem) {
			expire_name(adbname, DNS_ADB_CANCELED, now);
			removed++;
			goto next;
		}
...
again:
	...
	if (result == ISC_R_SUCCESS) {
		...
		if (dns_name_equal(name, &adbname->name)) {
			expire_name(adbname, DNS_ADB_CANCELED, INT_MAX);
		}
		UNLOCK(&adbname->lock);
		dns_adbname_detach(&adbname);
	}
