...
check_keys() {
	...
	_ret=0
	_count=$(cat K${_zone}*.private | grep Engine | wc -l)
	test "$_count" -eq "$_expect" || _ret=1
	_count=$(cat K${_zone}*.private | grep Label | wc -l)
	test "$_count" -eq "$_expect" || _ret=1
	test "$_ret" -eq 0 || echo_i "failed (expected Engine and Label in key files)"
	_status=$((_status+_ret))
...
EOF
	...
	$KEYGEN -E pkcs11 -k $alg -l named.conf $zone > keygen.out.$zone.$n 2>/dev/null || ret=1
	check_keys $zone 2 || ret=1
	status=$((status+ret))
